...
def test_mine_block():
    ...
    assert isinstance(block, Block) # Check if the block we created became an instance or object of Block class
...
def test_genesis():
    ...
    for key, value in GENESIS_DATA.items():
        getattr(genesis, key) == value # Checks if all values of genesis object are same as that in original dictionary
...
def test_slowly_mined_block():
    last_block = Block.mine_block(Block.genesis(), 'foo')
    time.sleep(MINE_RATE/SECONDS) # Delay to ensure it is mined slower than mine_rate, takes argument in second
    mined_block = Block.mine_block(last_block, 'bar')
...
# This is so that we don't have to create last_block and block for each test individually
@pytest.fixture
def last_block():
...
def test_is_valid_block_bad_last_hash(last_block, block):
    ...
    with pytest.raises(Exception, match='The block last hash must be correct'): # Tells python that we are expecting this part of code to raise an exception with given exception message
        Block.is_valid_block(last_block, block)
